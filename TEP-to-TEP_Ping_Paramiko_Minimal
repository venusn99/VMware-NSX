...
src_tep_ips = []
dst_tep_ips = []
log_file = datetime.datetime.now().strftime('tep2tep_ping_%d%m%y_%H_%M_%S.log') #log file name to send the output to logfile from functions.
...
def main():
    while True:
        try:
            ...
            delay_print="Warning ⚠️ pre-requisites: Atlease 1 VM/Test VM has to be present on each host connecting to NSX Overlay Segments\nhint: https://github.com/venusn99/pyvmomi-instant-VM-clone\nStarting the execution in:\n"
            for c in delay_print:
                sys.stdout.write(c)
                time.sleep(0.03)
                sys.stdout.flush()
...
def ssh_connect(ip, port, username, password,timeout_ssh):
    try:
        ...
        sys.exit(1)
# function to retrive source TEP IPs from Edge Nodes
    ...
    try:
        ...
        while True:
            ...
            if "Logical Router" in output_buffer:
                #print("Breaking loop as match found \"Logical Router\"") #GATE
                break
        #print(output_buffer) #GATE
        ...
        #print("source TEP IPs are: ", tep_ips) #GATE
        global src_tep_ips 
...
# function to retrive Destination TEP IPs from Edge Nodes
def get_dst_tep(ssh_connection):
    try:
        ...
        #print(tep_ips) #open tap to flow the data
        #print("Neighbor TEP IPs are: ", tep_ips) #GATE
        uniq_dst_tep_ips = list(set(tep_ips)) # prints unique tep IPs
        #print(f"The unique neighbor TEP IPs are: {uniq_dst_tep_ips} ") #GATE # Translating to set to get unique values and convert back to list to preserve the orginal order
        global dst_tep_ips 
